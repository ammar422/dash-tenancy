...
Route::macro('routelist', function () use ($DASHBOARD_PATH, $middlewares_dash, $domain) {
    Route::middleware($middlewares_dash)
        ->domain($domain)
        ->group(
            function () use ($DASHBOARD_PATH) {
                ...
                /////////////////////////// Guest Loggedin ////////////////////
                Route::get('login', [Authentication::class, 'index'])->middleware('dash.guest');
...
if (config('dash.tenancy.tenancy_mode', 'off') == 'on') {
    foreach (config('tenancy.central_domains', []) as $domain) {
        Route::routelist($DASHBOARD_PATH, $middlewares_dash, $domain);
    }
}

Route::routelist($DASHBOARD_PATH, ['web', \Dash\Middleware\SetLocale::class], '');
